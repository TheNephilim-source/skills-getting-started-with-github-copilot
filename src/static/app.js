  ...
  async function fetchActivities() {
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantItems = Array.isArray(details.participants) && details.participants.length > 0
          ? details.participants.map((participant) => `
              <li class="participant-item" data-activity="${name}" data-email="${participant}">
                <span>${participant}</span>
                <button class="delete-participant" type="button" title="Remove ${participant}" aria-label="Remove ${participant}">✕</button>
              </li>
            `).join("")
